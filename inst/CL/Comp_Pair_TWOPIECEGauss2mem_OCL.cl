...
/********************* SPATIAL CASE *****************************************************/
/******************************************************************************************/
__kernel void Comp_Pair_TWOPIECEGauss2mem_OCL(__global const double *data1,__global const double *data2,__global const double *mean1,__global const double *mean2, __global const double *lags, __global double *res,__global const int *int_par,__global const double *dou_par)
{
   ...
   if(!isnan(zi)&&!isnan(zj) )
    {
       ...
       // l2=one_log_two_pieceGauss(zj,mean2[gid],sill,eta);
        ...
        bl=log(biv_two_pieceGaussian((1-nugget)*corr,zi,zj,sill,eta,p11,mean1[gid],mean2[gid]));
