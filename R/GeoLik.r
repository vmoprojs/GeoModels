  ...
  LogNormDenStand_LG <- function(const,cova,ident,dimat,mdecomp,nuisance,det,sill,setup,stdata)
    {
        llik <- 1.0e8
        # Computes the covariance matrix:
        varcov <- (nuisance['nugget']+sill)*ident
        varcov[lower.tri(varcov)] <- cova
        varcov <- t(varcov)
        varcov[lower.tri(varcov)] <- cova      
        # decomposition of the covariance matrix:
...
################################################
        skew=nuisance["skew"]
        delta=nuisance["tail"]
        B=delta*cosh(delta*asinh(stdata)-skew)/sqrt(stdata^2+1)
        Z=sinh(delta * asinh(stdata)-skew)
        llik <- 0.5*( const + 2*const*log(sqrt(sill))/log(2*pi)
                      +logdetvarcov - 2*sum(log(B))
