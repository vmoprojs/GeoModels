...
GeoTests <- function(object1, object2, ..., statistic)
  {
      ...
      Isnested <- function(model1, model2)
      {
          ...
          if(identical(model1, "gencauchy"))
              if(identical(model2, "cauchy"))
                  result <- TRUE   
          if(identical(model1, "genwendland"))
              if(identical(model2, "Wend0")||identical(model2, "Wend1")||identical(model2, "Wend2"))
                  result <- TRUE               
      ...
      UnderNull <- function(model1, model2)
      {
          ...
          initparam <- StartParam(model2$coordx,model2$coordy,model2$coordt,model2$corrmodel,
          ...
          #numparam <- length(model2$par)#initparam$numparam
          ...
          GD=.C('GodambeMat',as.double(mm),as.integer(bivariate),as.double(model2$coordx),as.double(model2$coordy),
            ...
            as.integer(initparam$flagcorr),as.integer(initparam$flagnuis), as.integer(model2$grid),as.integer(initparam$likelihood),
             ...
             score=score,sensmat=sensmat,as.integer(spacetime),as.double(model2$threshold),as.integer(initparam$type),
      ...
      {   # consider the ith model:
          ...
          if(i>1){
            ...
            if((!all(names(lmodels[[j]]) %in% names(lmodels[[i]]))) &&
               (!identical(lmodels[[j]]$model,lmodels[[i]]$model))) # (!Isnested(lmodels[[j]]$corrmodel,lmodels[[i]]$corrmodel))
