...
pcl2=GeoFit(coordx=loc,corrmodel=corrmodel,data=z,
  ...
  maxdist=dd,optimizer="BFGS",start=start,fixed=fixed)
pcl2
...
KK=50
seed=9
a1=GeoCV(pcl1, K=KK, n.fold=0.25,seed=seed,sparse=TRUE)
a2=GeoCV(pcl2, K=KK, n.fold=0.25,seed=seed,sparse=TRUE)
...
pr1<-GeoKrig(loc=gr.in,coordx=loc,corrmodel=corrmodel,mse=TRUE,
  model="Gaussian",sparse=TRUE,
  param=as.list(c(pcl1$param,pcl1$fixed)),data=z)

pr2<-GeoKrig(loc=gr.in,coordx=loc,corrmodel=corrmodel,mse=TRUE,
  model="SkewGaussian",sparse=TRUE,
